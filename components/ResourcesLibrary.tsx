...
const ResourcesLibrary = () => {
    return (
        <div className="space-y-10 animate-fade-in pb-12 text-right">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {RESOURCES.map((cat, idx) => (
                    <div key={idx} className="space-y-4">
                        <h3 className="text-cine-accent font-bold flex items-center gap-2 text-xs uppercase tracking-wider justify-end">
                            {cat.category} <Book className="w-4 h-4" />
                        </h3>
                        <div className="space-y-3">
                            {cat.items.map((item, i) => (
                                <div key={i} className="glass-panel p-5 rounded-2xl border border-white/5 hover:border-cine-accent/20 transition-all hover:-translate-y-1 group">
                                    <div className="flex justify-between items-start mb-2">
                                        <span className="text-[10px] bg-white/5 text-slate-400 px-2 py-0.5 rounded border border-white/5">{item.type}</span>
                                        <h4 className="font-bold text-slate-200 text-sm group-hover:text-cine-accent transition-colors">{item.title}</h4>
                                    </div>
            ...
            <div className="glass-panel p-8 rounded-[30px] border border-white/5 mt-8 flex flex-col md:flex-row-reverse items-center md:items-start gap-6 text-center md:text-right">

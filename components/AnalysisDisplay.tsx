...
const AnalysisDisplay: React.FC<AnalysisDisplayProps> = ({ data }) => {
    return (
        <div className="animate-fade-in pb-12 text-right" dir="rtl">
            {/* Header */}
            <header className="text-center py-6 mb-8 animate-float">
                <div className="inline-block relative group">
                    <div className="absolute -inset-1 bg-gradient-to-r from-blue-600 to-transparent rounded-lg blur opacity-20 group-hover:opacity-40 transition duration-1000"></div>
                    <h1 className="relative text-3xl md:text-5xl font-extrabold tracking-tight mb-2 text-transparent bg-clip-text bg-gradient-to-r from-blue-200 to-blue-500">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <main className="lg:col-span-8 order-2 lg:order-1 space-y-8">
                    <article className="glass-panel rounded-[30px] p-1 overflow-hidden transition-all duration-500 hover:border-cine-accent/30 animate-glow">
                        {/* Inner Content */}
                        <div className="rounded-[28px] bg-gradient-to-br from-white/5 to-transparent p-6 md:p-10 h-full relative">

                            {/* Decorative Icon */}
                            <div className="absolute top-8 left-8 text-cine-accent/5 rotate-12 pointer-events-none">
                                <Quote size={120} />
                            </div>

                            <h2 className="text-xl font-semibold text-cine-accent mb-8 flex items-center gap-3 border-b border-white/5 pb-4 relative z-10">
                                <span className="w-2 h-2 rounded-full bg-cine-accent animate-pulse"></span>
                                الرؤية والتوجيه
                            </h2>

                            {/* Analysis Content (Font) */}
                            ...
                            <div className="my-10 relative z-10">
                                <div className="bg-gradient-to-r from-blue-900/20 to-transparent border-r-4 border-cine-accent p-8 rounded-l-2xl shadow-lg shadow-black/20">
                                    <div className="flex items-center gap-2 mb-4 text-cine-accent/80 text-xs font-bold uppercase tracking-wider">
                                        <Sparkles className="w-4 h-4" /> قبس من النور
                                    </div>
                                    <p className="font-serif text-3xl text-blue-100 mb-4 leading-relaxed drop-shadow-md">
                                        "{data.quranicLink.verse}"
                                    </p>
                                    <div className="flex flex-col md:flex-row md:items-center gap-3 text-sm text-cine-accent/70">
                                        <span className="font-bold border border-cine-accent/30 px-3 py-1 rounded-full">{data.quranicLink.surah}</span>
                                        <span className="hidden md:inline w-1 h-1 rounded-full bg-cine-accent/50"></span>
                    ...
                    {data.webAnalysis && (
                        <div className="glass-panel rounded-[30px] p-8 border border-blue-500/20 relative overflow-hidden">
                            ...
                            <div className="grid md:grid-cols-2 gap-6 mb-6">
                                <div className="bg-blue-900/10 p-5 rounded-2xl border border-blue-500/10 text-right">
                                    <h4 className="text-blue-300 font-bold text-sm mb-2 flex items-center gap-2 justify-end">
                                        <Search className="w-4 h-4" /> السبب الجذري (بحث)
                                    </h4>
                                    <p className="text-slate-300 text-sm leading-relaxed">{data.webAnalysis.rootCause}</p>
                                </div>
                                <div className="bg-emerald-900/10 p-5 rounded-2xl border border-emerald-500/10 text-right">
                                    <h4 className="text-emerald-300 font-bold text-sm mb-2 flex items-center gap-2 justify-end">
                            ...
                            {data.webAnalysis.sources && data.webAnalysis.sources.length > 0 && (
                                <div className="mt-4">
                                    <h4 className="text-xs uppercase tracking-wider text-slate-500 font-bold mb-3 border-b border-white/5 pb-2 text-right">نتائج البحث ذات الصلة</h4>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                        {data.webAnalysis.sources.map((source, idx) => (
                                            ...
                                            >
                                                ...
                                                <span className="font-bold text-blue-400 text-sm group-hover:underline mb-1 line-clamp-1 text-right">{source.title}</span>
                                                {source.snippet && <p className="text-[10px] text-slate-500 line-clamp-2 leading-relaxed text-right">{source.snippet}</p>}
                        ...
                        <div className="space-y-4 relative pr-4 border-r border-white/5">
                            {data.tomorrowPlan.map((plan, idx) => (
                                <div key={idx} className="relative pr-8 group">
                                    <div className={`absolute -right-[21px] top-4 w-4 h-4 rounded-full border-2 ${plan.type === 'study' ? 'bg-cine-dark border-emerald-500 group-hover:bg-emerald-500' : 'bg-cine-dark border-slate-600'} transition-colors shadow-lg shadow-black/50`}></div>
                                    <div className={`p-4 rounded-2xl border transition-all duration-300 ${plan.type === 'study' ? 'bg-white/5 border-white/5 hover:border-emerald-500/30 hover:bg-white/10' : 'bg-transparent border-transparent opacity-60'}`}>
                                        <div className="flex flex-wrap justify-between items-baseline mb-1 gap-2">
                                            <span className="font-bold text-slate-200 text-lg order-1">{plan.task}</span>
                                            <span className="text-xs font-mono text-emerald-500 bg-emerald-900/20 px-2 py-1 rounded order-2">{plan.time}</span>
                                        </div>
                                        {plan.method && (
                                            <div className="text-sm text-slate-400 flex items-center gap-2 mt-2 justify-end">
                                                {plan.method}
                                                <Zap className="w-3 h-3 text-cine-accent" />
                                            </div>
                                        )}
                                    </div>
                                </div>
                ...
                <aside className="lg:col-span-4 order-1 lg:order-2 space-y-6">
                    <div className="glass-panel rounded-3xl overflow-hidden text-right">
                        <div className="p-5 bg-white/5 border-b border-white/5 flex items-center justify-between">
                            <div className="flex items-center gap-2 text-slate-200">
                                ...
                                <Activity className="w-5 h-5 text-cine-accent" />
                            </div>
                        </div>
                                ...
                                {data.researchConnections.slice(0, 2).map((item, idx) => (
                                    <div key={idx} className="bg-black/30 p-3 rounded-xl border border-white/5">
                                        ...
                                        <div className="flex justify-between items-center text-[10px]">
                                            ...
                                            <span className="text-purple-400 flex items-center gap-1">{item.source} <BookOpen className="w-3 h-3" /></span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="pt-2">
                                ...
                                <div className="space-y-2">
                                    {data.recommendedMethods.map((m, i) => (
                                        <div key={i} className="flex items-center gap-2 text-xs text-slate-400 justify-end">
                                            <span>{m.subject}: <span className="text-slate-300">{m.methodName}</span></span>
                                            <CheckCircle2 className="w-3 h-3 text-cine-accent" />
                                        </div>
                                    ))}
                                </div>
                            </div>

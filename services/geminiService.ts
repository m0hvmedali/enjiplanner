...
export const analyzeDayAndPlan = async (
    ...
    3. قدم خطة للغد تراعي مواد ${gradeLevel} في مصر.
    ...
    return callWithRetry(async () => {
        try {
            const response = await ai.models.generateContent({
                model: "gemini-2.0-flash", // Using gemini-2.0-flash as a common and stable version
                contents: prompt,
                config: {
                    tools: [{ googleSearch: {} }],
...
export const getFreshInspiration = async (): Promise<MotivationalMessage> => {
    ...
    الشروط:
    1. استخدم Google Search للعثور على شيء جديد. 
    2. التنسيق المطلوب JSON فقط: { "text": "...", "source": "...", "category": "religious" | "scientific" | "wisdom" }
